...
| 中クリック | ロックオン切り替え |
| `P` | フォトモード（ポーズ＋自由飛行カメラ。`F12` と組み合わせて撮影） |
| `H` | HUD 表示切り替え |
| `F12` | スクリーンショット（`user://screenshots/` に保存） |
| `F1` | 画質プリセット切り替え（ULTRA → HIGH → BALANCED → PERFORMANCE） |
| `F9` | **60 秒自動デモモード**（シネマティックカメラ＋AI 操作。任意のキーで復帰） |
| `Esc` | マウスカーソル解放 |
| `R` | 死亡時のリスタート |
...
`F9`（またはコマンドライン `--demo`）で 60 秒のアトラクトモードが始まります。
6 カットのシネマティックリール（俯瞰オービット → 街路クレーン → ヒーロー
アーク → 戦闘カバレッジ → ローアングル → 引きのアウトロ）をカメラが流れ、
その間デジハリマンは AI ボットが操作します（接近・3 段斬撃・回避ダッシュ・
遠距離ボルトまでプレイヤーと同じコードパスで実行）。デモ中は被弾しても
致死にはならず（ダメージは HP 1 で頭打ち＋緩やかに回復）、死亡画面で
...
src/core/
  ...
  demo_director.gd       60 秒アトラクトモード（シーク可能なカメラリール＋ボット）
...
| `sky.gdshader` | カスタム大気：グラデーション・太陽・二層 FBM 雲・星空・地平の街明かり |
| `skyline.gdshader` | 遠景タワー群：窓グリッド発光＋空気遠近法フェード |
...
| プリセット | 想定 | 主な差分 |
|---|---|---|
| **ULTRA** | RTX 5080 / 1440p–4K | SDFGI 5 カスケード・SSIL・SSR 64・TAA + MSAA 2x・8192px 指向性影 |
...
- 13 本のシェーダがエンジンのパーサを通ること（ユニフォーム数で確認）
