...
- **カメラ**: クレーン/ドリー/トラッキング/オービットの9ショット + ショット別 DoF +
  レターボックス + 章テロップ + タイトル
- **サウンド**: 全編プロシージャル合成 (`tools/generate_audio.py`、生成済みoggを同梱)。
  デモのタイムラインに同期した60秒劇伴 (裂け目braam・バトルドラム・決意の進行)、
  フリープレイ用ループBGM、雨環境音、斬撃/ヒット/咆哮/巨獣の震脚などSFX 9種。
  Movie Maker が音声もAVIに記録し MP4 に AAC で同梱される
